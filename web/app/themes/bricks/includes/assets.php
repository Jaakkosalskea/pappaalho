...
class Assets {
	 ...
	 * Subkeys: breakpoints
	 */
	public static $inline_css_mobile = [];
	...
	public static function generate_inline_css( $post_id = 0 ) {
		...
		// STEP #5: Page header + content + footer
		...
		if ( ! empty( $content_template ) && is_array( $content_template ) ) {
			...
			if ( isset( Database::$active_templates[ $content_type ] ) ) {
				self::$page_settings_post_ids[] = Database::$active_templates[ $content_type ];
			}
	...
	public static function generate_css_color( $color ) {
		...
		if ( ! empty( $color['raw'] ) ) {
			return Integrations\Dynamic_Data\Providers::render_content( $color['raw'] );
		}
	...
	public static function generate_inline_css_theme_style( $settings = [] ) {
		...
		// Mobile CSS
		$inline_css .= self::generate_inline_css_mobile( 'theme_style' );
	...
	/**
	 * Generate inline CSS mobile for css_type (content, theme_style, etc.)
	 *
	 * Necessary to add mobile CSS of certain type to external CSS file
	 *
	 * @since 1.3.5
	 *
	 * @return string
	 */
	public static function generate_inline_css_mobile( $css_type ) {
		$inline_css = '';

		foreach ( Setup::$breakpoints as $breakpoint ) {
			$key       = $breakpoint['key'];
			$max_width = $breakpoint['max'];
			$value     = isset( self::$inline_css_mobile[ $css_type ][ $key ] ) ? self::$inline_css_mobile[ $css_type ][ $key ] : false;

			if ( $value ) {
				$css_type_uppercase = strtoupper( $css_type );

				$inline_css .= "\n/* $css_type_uppercase BREAKPOINT: $key */\n";
				$inline_css .= "@media (max-width: {$max_width}px) {\n";
				$inline_css .= $value;
				$inline_css .= '}';

				// STEP: Clear mobile breakpoint value to avoid generating duplicates (see: theme styles)
				unset( self::$inline_css_mobile[ $css_type ][ $key ] );
			}
		}
	...
	public static function load_webfonts( $inline_css ) {
		...
		foreach ( $google_fonts_families as $google_font ) {
			...
			if ( count( $font_weights ) ) {
				// Font weight(s) specified (i.e. 'Roboto:100,900' etc.)

				// If normal font weight '400' is not a font-weight: Add it manually
				if ( ! in_array( '400', $font_weights ) ) {
					$font_weights[] = '400';
				}

				sort( $font_weights );

				$google_font_family = $add_google_font . ':' . join( ',', $font_weights );
			} else {
				// No font weight(s) specified (i.e. 'Roboto')

				// Check for italic
				if ( strpos( $css_rules_string, 'font-style: italic' ) !== false ) {
					$add_google_font .= ':italic';
				}
	 ...
	 *
	 * @return array Key is CSS selector. Value is array of CSS rules for this CSS selector.
	 *
	 * @since 1.3.5
	 */
	public static function generate_css_rules_from_setting( $settings, $setting_key, $setting_value, $controls, $selector, $css_type ) {
				...
				if ( is_array( $setting_value ) ) {
					...
					switch ( $control_type ) {
								...
								switch ( $background_property ) {
									...
									case 'image':
										...
										if ( $dynamic_tag ) {
											...
											if ( $post_id ) {
												$background_size = ! empty( $background_value['size'] ) ? $background_value['size'] : BRICKS_DEFAULT_IMAGE_SIZE;
												$images          = Integrations\Dynamic_Data\Providers::render_tag( $dynamic_tag, $post_id, 'image', [ 'size' => $background_size ] );

												if ( isset( $images[0] ) ) {
													$background_url = is_numeric( $images[0] ) ? wp_get_attachment_image_url( $images[0], $background_size ) : $images[0];
												}
											}
										} else {
										...
										if ( $background_url || Database::get_setting( 'cssLoading' ) === 'file' ) {
											$css_rules[ $css_selector ][] = 'background-size: cover';
										}

										// Add dynamic data background image if not inside query loop (where its already added via inline style, @see base.php L1300)
										if ( $background_url && ! Query::is_looping() ) {
											// Add dynamic data via inline CSS (as we need the post ID of the requested post)
											if ( Database::get_setting( 'cssLoading' ) === 'file' && $dynamic_tag ) {
									...
									case 'size':
										...
										if ( $background_property === 'position' && $background_value === 'custom' ) {
											$background_position = [];
											...
											$css_rules[ $css_selector ][] = "background-$background_property: $background_value";
										}
										break;

									case 'custom':
										if ( isset( $setting_value['size'] ) && $setting_value['size'] === 'custom' ) {
						...
						case 'border':
							foreach ( $setting_value as $border_property => $border ) {
								switch ( $border_property ) {
									case 'width':
										$directions = isset( $control['directions'] ) ? $control['directions'] : [ 'top', 'right', 'bottom', 'left' ];

										foreach ( $directions as $direction ) {
											$value = isset( $border[ $direction ] ) ? $border[ $direction ] : '';
											$unit  = ! empty( $border['unit'][ $direction ] ) ? $border['unit'][ $direction ] : 'px';

											// Render '-' as unitless (default: 'px')
											if ( $unit === '-' || $unit === 'none' ) {
												$unit = '';
											}
									...
									case 'radius':
										...
										if ( count( $radius_rules ) === 4 && $border_radius ) {
						...
						case 'box-shadow':
							...
							$box_shadow_values = isset( $setting_value['values'] ) ? $setting_value['values'] : '';

							if ( $box_shadow_values ) {
								$box_shadow[] = isset( $box_shadow_values['offsetX'] ) ? $box_shadow_values['offsetX'] . 'px' : 0;
								$box_shadow[] = isset( $box_shadow_values['offsetY'] ) ? $box_shadow_values['offsetY'] . 'px' : 0;
								$box_shadow[] = isset( $box_shadow_values['blur'] ) ? $box_shadow_values['blur'] . 'px' : 0;
								$box_shadow[] = isset( $box_shadow_values['spread'] ) ? $box_shadow_values['spread'] . 'px' : 0;
							}
						...
						case 'dimensions':
							...
							foreach ( $directions as $direction ) {
								$value = isset( $setting_value[ $direction ] ) ? $setting_value[ $direction ] : '';
								$unit  = isset( $control['unit'] ) ? $control['unit'] : 'px';

								// NOTE: New since 1.2 (use any unit)
								if ( ! empty( $setting_value['unit'][ $direction ] ) ) {
									$unit = trim( $setting_value['unit'][ $direction ] );
								}
								...
								// Allow to use 'top', 'right', etc. directly in one control (see: mobileMenuToggleClosePosition)
						...
						case 'gradient':
							...
							elseif ( $setting_value['applyTo'] === 'overlay' ) {
								$css_selector .= '::before';
							}
							...
							elseif ( ! empty( $setting_value['angle'] ) ) {
								...
								switch ( $font_property ) {
									...
									case 'font-family':
										...
										if ( $custom_font_id ) {
											...
											// Add @font-face for custom fonts to inline CSS
											if ( get_post_type( $custom_font_id ) === BRICKS_DB_CUSTOM_FONTS && get_post_status( $custom_font_id ) === 'publish' ) {
												$font_weight = isset( $setting_value['font-weight'] ) ? intval( $setting_value['font-weight'] ) : '400';
												$font_style  = isset( $setting_value['font-style'] ) ? $setting_value['font-style'] : '';

												self::$inline_css['custom_fonts'] .= Custom_Fonts::generate_font_face_inline_css( $custom_font_id, $font_weight, $font_style );
											}
										}

										// Quote font-family (https://www.w3.org/TR/2011/REC-CSS2-20110607/fonts.html#font-family-prop)
					 ...
					 *
					 * @example repeat(%s, 1fr) to set mobile breakpoint CSS grid columns without any classes)
					 * @since 1.3
					 */
					if ( isset( $css_definition['value'] ) ) {
						if ( $css_property === 'content' ) {
							...
							$setting_value = Integrations\Dynamic_Data\Providers::render_content( $setting_value, $post_id );
						}
						...
						if ( ! empty( $css_definition['required'] ) && $setting_value !== $css_definition['required'] ) {
							...
							$setting_value = strpos( $css_definition['value'], '%s' ) !== false ? str_replace( '%s', $setting_value, $css_definition['value'] ) : $css_definition['value'];
						}
					}
		...
		if ( $breakpoint ) {
			foreach ( $css_rules as $css_selector => $css_declarations ) {
				...
				if ( ! isset( self::$inline_css_mobile[ $css_type ][ $breakpoint ] ) ) {
					self::$inline_css_mobile[ $css_type ][ $breakpoint ] = '';
				}

				self::$inline_css_mobile[ $css_type ][ $breakpoint ] .= $css_selector . ' {' . join( '; ', $css_declarations ) . '}' . PHP_EOL;
			}
	...
	public static function generate_inline_css_from_element( $element, $controls, $css_type ) {
		...
		if ( $element_id ) {
			...
			// CSS ID set
			elseif ( ! empty( $settings['_cssId'] ) ) {
				$css_selector = Query::is_looping() ? ".{$settings['_cssId']}" : "#{$settings['_cssId']}";
			}
		...
		foreach ( $css_rules as $css_selector => $css_declarations ) {
			...
			// @since 1.4: Merge multiple "background-image" declarations comma-separated into one (to use overlay/gradient & background image on same selector)
			// $background_images = [];

			// foreach ( $css_declarations as $index => $declaration ) {
			// if ( strpos( $declaration, 'background-image: ' ) !== false ) {
			// $declaration = str_replace( 'background-image: ', '', $declaration );

			// Prepend gradient to show it above background-image url
			// if ( strpos( $declaration, 'url(' ) === false ) {
			// array_unshift( $background_images, $declaration );
			// } else {
			// $background_images[] = $declaration;
			// }

			// unset( $css_declarations[$index] );
			// }
			// }

			// if ( count( $background_images ) ) {
			// $background_images = 'background-image: ' . ( implode( ', ', $background_images ) );
			// $css_declarations[] = $background_images;
			// }
		...
		if ( ! empty( $settings['_cssCustom'] ) && ! Query::get_loop_index() ) {
		...
		if ( $css_type !== 'theme_style' ) {
			$mobile_css  = self::generate_inline_css_mobile( $css_type );
			$inline_css .= $mobile_css . PHP_EOL;
		}
