...
class Element_Container extends Element {
	...
	public function set_controls() {
		...
		$this->controls['_innerContainerMargin'] = [
			...
			'css'   => [
				[
					'property' => 'margin',
					'selector' => '> .brxe-container, > .brxe-div',
				]
			],
		];
	...
	public static function get_shape_divider_html( $settings = [] ) {
		...
		foreach ( $shape_dividers as $shape ) {
			...
			if ( isset( $shape['height'] ) ) {
				$shape_inner_styles[] = "height: {$shape['height']}";
			}

			if ( isset( $shape['width'] ) ) {
				$shape_inner_styles[] = "width: {$shape['width']}";
			}
			...
			if ( isset( $shape['top'] ) ) {
				$shape_inner_styles[] = "top: {$shape['top']}";
			}

			if ( isset( $shape['right'] ) ) {
				$shape_inner_styles[] = "right: {$shape['right']}";
			}

			if ( isset( $shape['bottom'] ) ) {
				$shape_inner_styles[] = "bottom: {$shape['bottom']}";
			}

			if ( isset( $shape['left'] ) ) {
				$shape_inner_styles[] = "left: {$shape['left']}";
			}
	 ...
	 */
	public static function get_background_video_html( $settings ) {
		$background = ! empty( $settings['_background'] ) ? $settings['_background'] : false;
		...
		if ( strpos( $video_url, '{' ) === 0 ) {
			$video_url = Integrations\Dynamic_Data\Providers::render_tag( $video_url, get_the_ID(), 'link' );
		}
	...
	public function render() {
		...
		// Add .has-bg-video to container to set z-index: 1 (#2g9ge90)
		if ( ! empty( $settings['_background']['videoUrl'] ) ) {
		...
		$output .= self::get_background_video_html( $settings );
