...
class Helpers {
	...
	public static function get_registered_post_types() {
		...
		// Remove post type: 'attachment' (add to ControlQuery manually)
	 ...
	 * @return void
	 */
	public static function get_the_excerpt( $post, $num_words, $excerpt_more = null ) {
		$post = get_post( $post );
	...
	public static function get_posts_by_post_id( $query_args = [] ) {
		...
		// Query max. 50 posts to avoid running into any memory limits
	...
	public static function render_with_bricks( $post_id = 0 ) {
		...
		if ( ! Woocommerce::is_woocommerce_active() && function_exists( 'is_shop' ) && is_shop() ) {
			return false;
		}
	 ...
	 *
	 * Avoid file_get_contents() SSL error on local installs.
	 * Disable 'verify_peer' and 'verify_peer_name' is not a solution due to security (https://stackoverflow.com/a/26151993/2009539)
	 *
	 * On version 1.3 insert file_get_contents() as last resort if wp_remote_get() fails (servers without internal DNS)
	 *
	 * @return string
	 *
	 * @since 1.0
	 ...
	 *
	 * @since 1.3.5 (with backwards-compatibility using old 'responsive' repeater control)
	 */
	public static function generate_swiper_breakpoint_data_options( $settings ) {
		$slides_to_show   = ! empty( $settings['slidesToShow'] ) ? intval( $settings['slidesToShow'] ) : 1;
		$slides_to_scroll = ! empty( $settings['slidesToScroll'] ) ? intval( $settings['slidesToScroll'] ) : 1;

		$breakpoints = [];

		// @pre 1.3.5: Use "Responsive Breakpoints" repeater control
		...
		// @since 1.3.5: 'slidesToShow' & 'slidesToScroll' controls have 'breakpoints' => true to set those non-CSS controls directly for every breakpoint
		else {
			$base_breakpoint_slide_min_width = 1;

			foreach ( Setup::$breakpoints as $index => $breakpoint ) {
				$max_width = intval( $breakpoint['max'] );
				$min_width = isset( Setup::$breakpoints[ $index + 1 ]['max'] ) ? Setup::$breakpoints[ $index + 1 ]['max'] + 1 : 1;

				if ( $max_width > $base_breakpoint_slide_min_width ) {
					$base_breakpoint_slide_min_width = $max_width + 1;
				}

				if ( ! empty( $settings[ "slidesToShow:{$breakpoint['key']}" ] ) ) {
					$breakpoints[ $min_width ]['slidesPerView'] = $settings[ "slidesToShow:{$breakpoint['key']}" ];
				}

				if ( ! empty( $settings[ "slidesToScroll:{$breakpoint['key']}" ] ) ) {
					$breakpoints[ $min_width ]['slidesPerGroup'] = $settings[ "slidesToScroll:{$breakpoint['key']}" ];
				}
			}

			// Set base breakpoint slides to show
			if ( $slides_to_show != 1 ) {
				$breakpoints[ $base_breakpoint_slide_min_width ]['slidesPerView'] = $slides_to_show;
			}

			// // Set base breakpoint slides to scroll
			if ( $slides_to_show != 1 ) {
				$breakpoints[ $base_breakpoint_slide_min_width ]['slidesPerGroup'] = $slides_to_scroll;
			}
		}
