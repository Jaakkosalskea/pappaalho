...
class Woocommerce_Products_Filters extends Element {
	...
	public function render_control_dropdown( $filter, $filter_by ) {
		...
		$current_value = is_array( $current_value ) ? current( $current_value ) : $current_value;
	...
	public function render_control_slider( $filter, $filter_by ) {
		...
		// Current value.
		$value_min = absint( isset( $_GET[ $min_arg ] ) ? wp_unslash( $_GET[ $min_arg ] ) : $min_value );
		$value_max = absint( isset( $_GET[ $max_arg ] ) ? wp_unslash( $_GET[ $max_arg ] ) : $max_value );
